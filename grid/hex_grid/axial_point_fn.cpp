...
namespace
{
	std::size_t get_neighbor_count(std::uint32_t range)
	{
		/*
		...
		*/
		return std::size_t{3} * range * (std::size_t{range} + 1);
	}
}
...
namespace grid::hex
{
	std::vector<axial_point> get_neighbors(axial_point const & origin, std::int32_t range)
	{
		...
		std::size_t neighborCount = get_neighbor_count(static_cast<std::uint32_t>(n));
